...
mol = osprey.readPdb('/hpc/home/etm33/kinase_inhibitor_design/structures/processed/postMFSaf3complex.pdb') #pkn2.cleaned.filtered.pdb')
...
flex_residues = ['B2', 'B8', 'B10']

for res in flex_residues:
    ligand.flexibility[res].setLibraryRotamers(osprey.WILD_TYPE).addWildTypeRotamers().setContinuous()

#ligand.flexibility['B1'].addWildTypeRotamers().setContinuous()
#ligand.flexibility['B12'].addWildTypeRotamers().setContinuous()
...
kstar = osprey.KStar(
	...
	writeSequencesToFile='secondmfs.kstar.results.tsv'
)
